...
int main()
{
	...
	sf::Text PointOut, LoseOut, PauseOut, Start, Start1, Nazwa, Exit, Exit1, Back, Back1, score, TryAgain, TryAgain1;
	sf::Sprite sBackground(t1), sPers(t3), sMenu(t4);
	sf::Font font;
	...
	while (app.isOpen())
	{
		...
		if (menu == 1)
		{
			...
			if (Mouse_Position.x >= 170 && Mouse_Position.x <= 220 && Mouse_Position.y >= 445 && Mouse_Position.y <= 470)
			{
				app.draw(Exit1);
			...
			if (Mouse_Position.x >= 113 && Mouse_Position.x <= 298 && Mouse_Position.y >= 243 && Mouse_Position.y <= 273)
			{
				app.draw(Start1);
				if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
				{
		...
		if (menu == 2)
		{
			...
			if (!pause)
			{
				...
				if (plat1.lose() && plat2.lose() && plat3.lose() && plat4.lose() && plat5.lose() && plat6.lose() && plat7.lose() && plat8.lose() && plat9.lose() && plat10.lose())
				{
					app.draw(LoseOut);
					app.draw(Exit);
					app.draw(Back);
					...
					if (Mouse_Position.x >= 115 && Mouse_Position.x <= 285 && Mouse_Position.y >= 395 && Mouse_Position.y <= 420)
					{
						app.draw(Back1);
						if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
						{
					...
					if (Mouse_Position.x >= 130 && Mouse_Position.x <= 270 && Mouse_Position.y >= 345 && Mouse_Position.y <= 370)
					{
						app.draw(TryAgain1);
						if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
						{
